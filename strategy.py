...
def combo_ma_stoch(day_data):
    ...
    rsiAction = stoch_check(day_data)
    if rsiAction == Action.SELL and moving_avg_check(day_data) == Action.SELL:
        return Action.SELL
    elif rsiAction == Action.BUY and moving_avg_check(day_data) == Action.BUY:
        return Action.BUY
    return Action.HOLD